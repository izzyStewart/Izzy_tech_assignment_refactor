...
skills.drop(
    "conceptType", axis=1, inplace=True
)  # Removing first column as only one value

# Add new columns removing stop words and lowercasing skill label and description columns
skills["Label_without_stopwords"] = skills["preferredLabel"].apply(
    lambda x: " ".join([word for word in x.split() if word not in (stop)])
)
skills["Label_without_stopwords"] = skills[
    "Label_without_stopwords"
].str.lower()  # Lowercase column
skills["Desc_without_stopwords"] = skills["description"].apply(
    lambda x: " ".join([word for word in x.split() if word not in (stop)])
)
skills["Desc_without_stopwords"] = skills[
    "Desc_without_stopwords"
].str.lower()  # Lowercase column

# Clean alt labels column
skills["altLabels"] = skills["altLabels"].replace("\n", " ", regex=True)
# Create text merge column
skills["text_col"] = (
    skills["preferredLabel"] + " " + skills["altLabels"] + " " + skills["description"]
).astype(str)
skills["text_col"] = skills["text_col"].str.lower()  # Lowercase column
...
skills.drop(["Label_without_stopwords", "Desc_without_stopwords"], axis=1, inplace=True)
...
skills.drop(
    ["conceptUri", "text_col"], axis=1, inplace=True
...
stop = stopwords.words("english")  # stop word load
